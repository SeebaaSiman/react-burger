...
const AddProducts = () => {
  const { mockapiProducts, onModal, onCloseModal, openModal, createProduct } = useAdmin();
  return (
    <AddProductsStyle>
      <span>
        <h3>Cantidad de productos {mockapiProducts.length}</h3>
        <button onClick={onModal}>
          ...
          <p>Subir product nuevo</p>
        </button>
      </span>
      {openModal && <FormEditProduct clickedItem={obj_value} fn={createProduct} onCloseModal={onCloseModal} />}
      {mockapiProducts.map((item, index) => (
        <div key={index}>
          <div>
            <p>{item.name}</p>
...
const AddProductsStyle = styled.section`
  padding: 16px;
  display: flex;
  ...
  div {
    border-bottom: 1px solid ${(props) => props.theme.thirdText};
  }
  span {
    margin-bottom: 12px;
  }
